...
export default function Testimonials() {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      ...
      try {
        ...
        console.log("TESTIMONIAL API RESULT:", result);

        if (response.ok && Array.isArray(result.data)) {
          const formatted = result.data.map((item) => ({
            name: item.name,
            role: item.designation || "Student",
            photo: item.avatar || "/assets/images/gaura/images.png",
          ...
          console.error("Invalid testimonial response:", result);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      ...
      >
        <div className="w-full max-w-6xl mx-auto px-6 sm:px-10 text-center">
  ...
  return (
    ...
    >
      ...
      <div className="section-visible w-full max-w-6xl mx-auto px-6 sm:px-10">
        ...
        >
          <span className="big-quote" aria-hidden>
            "
          </span>
        ...
        <div className="flex items-center justify-center gap-3 mt-8 flex-wrap">
          {testimonials.map((s, i) => (
            <button
              ...
              onClick={() => goTo(i)}
              aria-label={s.name}
              className={`thumb-btn rounded-full overflow-hidden ${
                i === active ? "active-thumb" : ""
              }`}
              style={{
                width: "44px",
                height: "44px",
